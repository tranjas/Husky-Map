...
public class DoubleMapMinPQ<T> implements ExtrinsicMinPQ<T> {
    ...
    public T removeMin() {
        ...
        double minPriority = priorityToItem.firstKey(); //get the min priority
        Set<T> itemsWithMinPriority = priorityToItem.get(minPriority); //get the set -> priorty and the item
    ...
    public void changePriority(T item, double priority) {
        ...
        double oldPriority = itemToPriority.get(item); //constant
        if (priority != oldPriority) {
            Set<T> itemsWithOldPriority = priorityToItem.get(oldPriority); //log(N)
            itemsWithOldPriority.remove(item); //log(N)
